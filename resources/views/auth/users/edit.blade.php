    ...
    <form method="post" class="flex flex-col gap-8 w-full">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="flex flex-col">
                ...
                <input type="password" name="password" class="flex-auto border border-neutral-400 bg-white rounded-md py-2 px-4 @error('password') border-red-500 @enderror">            
                @error('password') <span class="text-red-500">{{ $message }}</span> @enderror
            </div>

            <div class="flex flex-col">
                <label>Confirm password</label>
                <input type="password" name="password_confirmation" class="flex-auto border border-neutral-400 bg-white rounded-md py-2 px-4 @error('password_confirmation') border-red-500 @enderror">
                @error('password') <span class="text-red-500">{{ $message }}</span> @enderror
            </div>
        </div>
